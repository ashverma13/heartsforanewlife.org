...
<div class="mod_caroufredsel mod_caroufredsel__<?php echo $moduleclass_sfx; ?>" id="module_<?php echo $module->id; ?>">
	...
	<?php if ($params->get('navigation') && $key>0): ?>
		<div id="carousel_<?php echo $module->id; ?>_prev" class="caroufredsel_prev"></div>
		<div id="carousel_<?php echo $module->id; ?>_next" class="caroufredsel_next"></div>
	<?php endif; ?>
...
<script>
if ((jQuery.browser.msie && jQuery.browser.version<10)||!jQuery("#caroufredsel_<?php echo $module->id; ?>").find('img').length) {
	jQuery(function() {
		jQuery("#caroufredsel_<?php echo $module->id; ?>").carouFredSel({
			responsive	: true,
			width: '100%',
			items		: {
